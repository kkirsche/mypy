...
@click.option(
    "--action", "-a", type=click.Choice(actions), required=True, help="What do I have to do :-)"
)
...
@click.option(
    ...
    "extension",
    default=".py",
    help='Extension "from" will be applied the action. Default .py',
)
@click.option(
    "--to",
    "-t",
    "to_extension",
    default=".pyi",
    help='Extension "to" will be applied the action if can. Default .pyi',
)
@click.option(
    "--exclude",
    "-x",
    multiple=True,
    default=("__init__",),
    help="For every appear, will ignore this files. (can set multiples times)",
)
@click.option(
    "--not-recursive",
    "-n",
    default=True,
    is_flag=True,
    help="Set if don't want to walk recursively.",
)
def main(
    action: str,
    directory: str,
